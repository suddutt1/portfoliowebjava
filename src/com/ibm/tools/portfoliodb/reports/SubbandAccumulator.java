...
public class SubbandAccumulator implements ReportEntryAccumulator {
	...
	private void addToTotalCount(String category,Double value)
	{
		...
		{
			//Add all the entries
			List<ReferenceData> refDataList = DataCache.getSubBandList();
	...
	private void addColumnEntry(Map<String, Double> lineData,
			ProjAssignmentDetails rptData) {
		String safeColumnName = getSafeString(rptData.getEmpDetails().getSubBand());
