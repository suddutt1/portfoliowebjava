...
public class LocationAccumulator implements ReportEntryAccumulator {
	...
	private void addToTotalCount(String category,Double value)
	{
		...
		{
			//Add all the entries
			List<ReferenceData> refDataList = DataCache.getLocationList();
	...
	{
		String location = rptData.getEmpDetails().getLocation();
		if(location!=null)
		{
				Double count = lineData.get(location);
				if(count==null)
				{
