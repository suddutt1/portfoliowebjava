...
public class MobileFrameworkDispatcher extends HttpServlet 
{
	...
	private static final Logger LOGGER 				= Logger.getLogger(MobileFrameworkDispatcher.class.getName());
	...
	public void init(ServletConfig config) throws ServletException 
	{
		...
		String[] listOfClasses = loadActionClassNames(config.getInitParameter("applicationConfigLocation"));
		
		if (listOfClasses != null && listOfClasses.length > 0) 
		{
			LOGGER.log(Level.INFO, "|MOBILE_FRMWK_DISPATCHER|List of classes read . Size is :" + listOfClasses.length);
			this.actionConfig = MobileFrameworkHelper.loadActionConfigs(listOfClasses);
		} 
		else 
		{
			LOGGER.log(Level.WARNING, "|MOBILE_FRMWK_DISPATCHER|List of classes is empty");
		}
	}
	 ...
	 * @throws IOException
	 */
	private void performProcessing(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		try 
		{
			if (this.actionConfig != null) 
			{
				...
				Class<? extends WebActionHandler> actionClass = this.actionConfig.getActionClass(actionName);

				WebActionHandler actionInstance = getObjectInstance(actionClass);
				ModelAndView result = (ModelAndView) method.invoke(actionInstance, new Object[] { request, response });
				if (result != null) 
				{
